...
В ТЗ формат ошибок был описан в общем виде, но не было явной обработки ошибок парсинга тела запроса. Я добавил тип RespError и код BAD_REQUEST в виде доменной ошибки (errs.ErrBadRequest, errs.ErrInvalidJSON) и ввёл единый маппер mapErrorToHTTPResponse, который конвертирует доменные ошибки в коды из OpenAPI (TEAM_EXISTS, PR_EXISTS, NOT_FOUND, BAD_REQUEST и т.п.), возвращает корректный HTTP-статус (400/404/409), для всех «падений» и валидации возвращает { "error": { "code": "BAD_REQUEST", ... } }, а не голый текст.
...
В задании не было кристально ясно, из какой команды брать нового ревьювера при переназначении.
Я принял решение: при обычном reassignment и при массовой деактивации брать кандидатов из команды автора PR, исключать: самого автора, уже назначенных ревьюверов, деактивируемых пользователей.
Это делает логику консистентной с назначением ревьюверов при создании PR (там тоже выбор идёт из команды автора).
